...
rollback() {
    ...
    if (( SWAPPED )); then
        ...
        if [[ -d "$APP_DIR" && "$APP_DIR" == /opt/dvswitch_mode_switcher ]]; then rm -rf -- "$APP_DIR"; fi
...
chmod 0755 "$STAGE/install-dvs-mode-switcher.sh" "$STAGE/installer/dvswitch-dmr-network" "$STAGE/installer/restore-backup"
...
log "Removing installation-only files from the production application"
rm -rf -- "$APP_DIR/.git" "$APP_DIR/tests" "$APP_DIR/installer" "$APP_DIR/presets"
rm -f -- "$APP_DIR/.gitattributes" "$APP_DIR/.gitignore" "$APP_DIR/CHANGELOG.md" "$APP_DIR/README.md" "$APP_DIR/install-dvs-mode-switcher.sh" "$APP_DIR/package-lock.json" "$APP_DIR/configs/config.example.yml"
[[ -f "$APP_DIR/index.js" && -f "$APP_DIR/package.json" && -d "$APP_DIR/node_modules" && -d "$APP_DIR/models" && -d "$APP_DIR/modules" && -d "$APP_DIR/public" && -d "$APP_DIR/views" ]]
[[ -f "$APP_DIR/LICENSE" && -f "$APP_DIR/configs/config.yml" && -f "$APP_DIR/configs/tg_alias.yml" ]]
systemctl is-active --quiet "$SERVICE"
curl -fsS --max-time 5 http://127.0.0.1:3000/ >/dev/null
if [[ -n "$OLD_APP" && -d "$OLD_APP" && "$OLD_APP" == /opt/dvswitch_mode_switcher.before-* ]]; then rm -rf -- "$OLD_APP"; OLD_APP=; fi
